---
title: Horn Players & Ensembles
description: A survey of contemporary horn players and ensembles in jazz.
order: 32
media:
  snarky-puppy:
    youtube: j_rOAmnISzE
    params: 'list=FLmQ5d-a1feza-7q3-3CQitQ'
    title: 'Snarky Puppy video'
  sfjazz-recording-session:
    youtube: bBBZ-2PzSgY
    title: 'SFJAZZ Collective recording session'
intro: |-
  In this unit, we explore contemporary horn players (beyond saxophonists) and ensembles that have shaped the modern jazz landscape. Some of these artists specialize in older, New Orleans-inspired traditions; others push the boundaries with fusion, funk, and cross-genre collaborations. We’ll also look at larger ensembles that continue evolving jazz orchestration, arrangements, and repertoire.
sections:
- heading: Wycliffe Gordon
  artists:
  - name: Wycliffe Gordon
    born: 1967
  prose: |-
    Wycliffe Gordon is a trombonist, composer, bandleader, and educator. [Read his website biography](https://wycliffegordon.com/bio/) for a full list of his accomplishments. He gained prominence with Wynton Marsalis’s band (1989–1995) and was a founding member of the Jazz at Lincoln Center Orchestra. He even arranged NPR’s “All Things Considered” theme.

    Gordon often focuses on early jazz styles, employing “tailgate” trombone techniques and plunger mutes reminiscent of Joe “Tricky Sam” Nanton. He’s heavily influenced by Louis Armstrong, having recorded multiple albums dedicated to Armstrong’s music.
  tracks:
  - title: St. Louis Blues
    performer: Wycliffe Gordon Ensemble
    composer: W.C. Handy
    recorded: '2012'
    album: Dreams of New Orleans
    personnel:
    - [Wycliffe Gordon, trombone]
    - [Jon-Erik Kellso, trumpet]
    - [Adrian Cunningham, soprano saxophone]
    - [Matt Munisteri, banjo]
    - [Michael Dease, baritone saxophone]
    - [Ibanda Ruhumbika, tuba]
    - [Marion Felder, drums]
    spotify: 7xn9IVBQCVzeybnkLEgbsi
    intro: |-
      Form: It's a blues with a bridge. It's often AABA, where each A is a 12 bar blues, and the B is a 16 bar habanera in a minor key. Sometimes musicians invert this, however, playing BAA instead. In this track, they play with the form, extending a single section as long as the soloist needs.
    guidenote: |-
      **Form:** A variation on AABA form with blues sections and a minor-key habanera. They manipulate form as needed for solos.
    guide: |-
      0:00 | B section
      0:31 | AA (12-bar blues x2)
      1:17 | Gordon (tbn) “talking” solo over multiple B sections
      2:15 | Cunningham (sop) solo over A
      2:59 | Kellso (tp) solo
      3:45 | Dease (bar) solo
      4:29 | Head out from B
  notes: |-
    - We’ve heard “St. Louis Blues” by Bessie Smith and Ella Fitzgerald earlier in the course. This is a modern take on an old standard.
    - This track shows that older styles remain part of the contemporary toolkit.
- heading: Dave Douglas
  artists:
  - name: Dave Douglas
    born: 1963
  prose: |-
    Dave Douglas is a trumpeter, composer, and educator. [Read his Grove article](https://libproxy.library.unt.edu:2147/10.1093/gmo/9781561592630.article.A2228230) . He attended Berklee, NEC, and NYU. He played with Horace Silver in the 80s and has earned numerous awards, including multiple Trumpet Player of the Year awards from *DownBeat* and Jazz Journalists Association.

    Douglas runs [Greenleaf Music](https://greenleafmusic.com) , a label and online hub for jazz, releasing music by Linda Oh, Donny McCaslin, Kneebody, and others. He also hosts the “A Noise from the Deep” podcast.
  tracks:
  - title: Deep River
    performer: Dave Douglas Quintet
    composer: Traditional African American spiritual
    recorded: February 2, 2015
    album: Brazen Heart
    personnel:
    - [Dave Douglas, trumpet]
    - [Jon Irabagon, tenor saxophone]
    - [Matt Mitchell, piano]
    - [Linda Oh, bass]
    - [Rudy Royston, drums]
    spotify: 5BcnuWz8cbRAgkKoXU18sW
    intro: |-
      Form: AABA'. The final A is much shorter than the first two. Maybe more like a tag or a coda.
    guidenote: |-
      **Form:** AABA’, with a shorter final A.
    guide: |-
      0:00 | A, head in (tp melody, ts counters)
      0:33 | A
      1:03 | B (Irabagon melody, Douglas counters)
      1:35 | A' (short final A)
      1:51 | Oh (b) solo on A
      2:24 | Mitchell (p) solo on A
      2:57 | Irabagon (ts) solo on B, with Douglas line behind
      3:29 | Head out on final A
  notes: |-
    - Watch the filmed recording session on YouTube.
    - “Deep River” is a 19th-century spiritual. Compare this interpretation with a traditional version.
    - Linda Oh is a rising star on bass, featured on many contemporary albums.
- heading: Terence Blanchard
  artists:
  - name: Terence Blanchard
    born: 1962
  prose: |-
    Terence Blanchard, from New Orleans, studied at Rutgers and took Wynton Marsalis’s place in the Jazz Messengers in 1982. He’s a prolific film composer (notably for Spike Lee) and contributed to Disney’s *The Princess and the Frog* . With multiple Grammys and nominations, Blanchard bridges jazz, film music, and activism.

    His 2015 album *Breathless* (with the E-Collective) was dedicated to Eric Garner. The 2018 follow-up, *Live* , addressed gun violence. Check out the documentary-style video (segments at 0–4:38, 10:52–..., 23:48–...).
  tracks:
  - title: Soldiers
    performer: Terence Blanchard & The E-Collective
    composer: Terence Blanchard
    recorded: '2018'
    album: Live
    personnel:
    - [Fabian Almazan, 'piano, keyboards']
    - [Charles Altura, guitar]
    - [Terence Blanchard, 'trumpet, synthesizers']
    - [David Ginyard, bass]
    - [Oscar Seaton, drums]
    spotify: 45qc0mhB3SrAQ4Qebc9a7p
    guide: |-
      0:00 | Drums groove
      0:23 | Bass, guitar with wah, radio chatter samples
      0:53 | Blanchard melody (effects-laden)
      1:36 | Altura (g) solo
      4:48 | Band hits end guitar solo
      4:57 | Blanchard (tp) solo with reverb/delay effects
      7:09 | Radio chatter returns, Blanchard stops solo
  notes: |-
    - The E-Collective also performed with UNT guitar faculty Davy Mooney; check the provided YouTube link.
- heading: Snarky Puppy
  prose: |-
    Snarky Puppy is a large, rotating collective founded by bassist, composer, and UNT alumnus Michael League. They began as a group of UNT students, later fusing with Dallas gospel/R&B communities. Featuring musicians like Robert “Sput” Searight and Shaun Martin, they blend jazz, funk, R&B, and more, winning multiple Grammys.

    @video(snarky-puppy)
  tracks:
  - title: Lingus
    performer: Snarky Puppy
    composer: Michael League
    recorded: October 2013
    album: We Like it Here (2014)
    personnel:
    - [Michael League, bass]
    - ['Jay Jennings, Mike Maher', trumpet/flugelhorn]
    - [Chris Bullock, 'tenor saxophone, bass clarinet, flute']
    - [Bob Reynolds, tenor saxophone]
    - ['Yannick Hiwat, Tessel Hersbach', violin]
    - [Mara Tieles, viola]
    - [Susanne Rosmolen, cello]
    - [Bill Laurance, 'piano, keyboards']
    - [Justin Stanton, 'keyboards, trumpet']
    - [Cory Henry, keyboards]
    - [Shaun Martin, 'Moog talkbox, keyboards']
    - ['Bob Lanzetti, Mark Lettieri, Chris McQueen', guitars]
    - [Larnell Lewis, drums]
    - ['Nate Werth, Julio Pimental, Steven Brezet', percussion]
    spotify: 68d6ZfyMUYURol2y15Ta2Y
    intro: |-
      Form: It's long and, although there are repeating sections, it's somewhat through-composed
    guidenote: |-
      **Form:** Complex and somewhat through-composed, with repeated thematic sections (A, B, C) and extensive solos.
    guide: |-
      0:00 | Intro groove (Rhodes w/ phaser effect)
      0:08 | A (Horns melody)
      0:46 | B
      1:00 | C (guitar)
      1:19 | A
      1:56 | B
      2:29 | C
      2:47 | C with new groove
      3:04 | Sax & trumpet trade over C
      3:57 | C melody again
      4:15 | Breakdown, Cory Henry readies for solo
      4:57 | Cory Henry’s epic synth solo
      8:11 | Band hits (2-3 clave?), horns play lines, Henry solos between phrases
      9:52 | End, applause
  notes: |-
    - Watch the original YouTube video of this performance. The energy and interplay are best experienced visually.
- heading: SFJazz Collective
  prose: |-
    The SFJazz Collective is the resident ensemble of SFJazz, formed in 2004 by Joshua Redman. They focus annually on the music of a jazz great. Their rosters have included top-tier musicians like Miguel Zenón, Avishai Cohen, Mark Turner, Stefon Harris, and many others. Each year, they also commission new works from the members.

    In 2011, they recorded an album featuring the music of Stevie Wonder alongside new compositions.
  tracks:
  - title: Superstition
    performer: SFJazz Collective
    composer: Stevie Wonder
    arranger: Miguel Zenón
    recorded: March 30-April 3, 2011
    album: Music of Stevie Wonder and New Compositions
    personnel:
    - [Avishai Cohen, trumpet]
    - [Robin Eubanks, trombone]
    - [Miguel Zenon, alto saxophone]
    - [Mark Turner, tenor saxophone]
    - [Stefon Harris, vibraphone]
    - [Edward Simon, piano]
    - [Matt Penman, bass]
    - [Eric Harland, drums]
    spotify: 3xvMAp0XEjYTQRuEu697ts
  notes: |-
    - All these musicians are notable individually. Zenón’s arrangement deconstructs Wonder’s original, adding complex harmonies and rhythms.
    - Watch the video of the recording session on YouTube to fully appreciate the interplay and arrangement:

    @video(sfjazz-recording-session)
---
