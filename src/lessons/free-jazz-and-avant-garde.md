---
title: Free Jazz and the Avant Garde
description: A history of jazz styles and figures through selected recordings
order: 19
intro: |-
  “Free jazz might be witty, even funny, but it wasn’t entertainment; it was a serious, challenging music, requiring the listener’s full concentration – art for art’s sake.” [(Deveaux & Giddins, p.309)](bibliography.html#deveaux-giddins-2011)

  Associated with a club in NY called the **Five Spot** . Its first official engagement was Cecil Taylor. In 1959, it hosted Thelonious Monk (we heard a recording from that gig). Later, it famously hosted Ornette Coleman.
sections:
- heading: Ornette Coleman
  artists:
  - name: Ornette Coleman
    born: 1930
    died: 2015
  prose: |-
    Called his approach **“harmolodic”** – harmony + movement + melody. As Coleman described it in the liner notes of *Dancing in Your Head* , "This means the rhythms, harmonics and tempos are all equal in relationship and independent melodies at the same time."

    [Here's a blog post that addresses the concept of "harmolodics."](http://harderbop.blogspot.com/2012/10/defining-harmolodics.html)

    One of the great innovators in jazz.

    Promoted by MJQ members John Lewis and Percy Heath.

    Grew up in Fort Worth, TX.

    Don Cherry plays an odd instrument called a pocket cornet (he called it a "pocket trumpet"). It's like a trumpet but, as its name implies, much smaller.

    Gig at the Five Spot in 1959 one month after release of *The Shape of Jazz to Come* . Here, Mingus and Miles saw him play (as did many others). Scheduled to last two weeks. Got extended to two-and-a-half months.

    Mingus and Davis were critical. “It’s like not having anything to do with what’s around you, and being right in your own world” - Mingus
  tracks:
  - title: Lonely Woman
    performer: Ornette Coleman
    composer: Ornette Coleman
    recorded: May 22, 1959
    album: The Shape of Jazz to Come
    personnel:
    - [Ornette Coleman, alto saxophone]
    - [Don Cherry, cornet]
    - [Charlie Haden, bass]
    - [Billy Higgins, drums]
    spotify: 23aDP04QX8MAHA99FLaPtN
    intro: |-
      No chordal instrument. A jazz standard (one of the oddest?). Ornette Coleman is considered a founding figure of the "free jazz" style, and this album, *The Shape of Jazz to Come* , is one of the most influential albums of the style.

      - Things "Lonely Woman" has in common with traditional jazz:
        - It begins and ends with the same melody, which has the form AABA.
        - The form is head, solos, head.
        - The drummer (Higgins) plays a familiar swing pattern.
        - The instrumentation is typical for a jazz quartet, with the notable absence of a chordal instrument (e.g., piano or guitar).
      - Things “Lonely Woman” does differently:
        - The number of measures in each section is not predetermined; the musicians stretch or compress the length of each section as they wish.
        - There is no clearly defined harmony - no specified chords.
        - The bass player does not play a walking pattern.
    guide: |-
      0:00 | Intro, Higgins (drums) and Haden (bass) begin the song together. At first the drums play only sparsely. About 7 seconds in, Higgins begins to play a more rapid swing pattern. Haden plays two notes at a time on the bass, a technique called a **double-stop** .
      0:19 | Head In, First A - Coleman and Cherry play the melody together in octaves. The melody seems to float above the accompaniment, not quite in time, but not quite out-of-time either.
      0:44 | Head In, Second A
      1:09 | Head In, B - Coleman plays the B section of the head. Cherry and Haden play rising chromatic notes that build tension and create an ominous mood.
      1:22 | Head In, final A - Coleman and Cherry again play the A section together, in octaves.
      1:47 | Solo, A - Coleman takes a solo over the A section of the song. There is no set number of measures per section, so this section is of an indeterminate length. We can tell that he's soloing over the A section because Haden is playing the same notes on the bass that he played during the A sections of the head.
      2:21 | Solo, B - The beginning of the B section is heralded by the rising chromatic notes played by Cherry and Haden.
      2:34 | Solo, A - Coleman continues his solo over another A section. Again, there is no clearly defined number of measures, but Haden's use of double stops on a familiar (tonic) note signals that the musicians are playing the A section.
      2:55 | Head Out, AABA - As we expect from jazz songs, the piece ends with a restatement of the melody. It is played in mostly the same manner as the head in was.
      4:20 | Outro - Coleman and Cherry wail on their instruments for a brief time before dropping out. The piece ends how it began: with bass and drums alone. Cherry makes a mistake at 4:03.
- heading: Eric Dolphy
  artists:
  - name: Eric Dolphy
    born: 1928
    died: 1964
  prose: |-
    Played alto saxophone, clarinet, bass clarinet, flute.

    Collaborated with Mingus.

    From Los Angeles.

    Goal was to make instruments speak like a human voice.
  tracks:
  - title: Out to Lunch
    performer: Eric Dolphy
    composer: Eric Dolphy
    recorded: February 25, 1964
    album: Out to Lunch
    personnel:
    - [Eric Dolphy, alto saxophone]
    - [Freddie Hubbard, trumpet]
    - [Bobby Hutcherson, vibraphone]
    - [Richard Davis, bass]
    - [Tony Williams, drums]
    spotify: 6BD4u2tOTHMlACAZ8gX0Gs
    intro: |-
      Dolphy died only a few months after this album was recorded.

      From liner notes to album: “This is a recurring figure around an improvised chorus. This figure, in 5/4, sets the rhythm section up with a definite solo feeling. In the improvised sections, the rhythms overlap. The bass follows no bar line at all. Notice Tony [Williams]. He doesn’t play time, he plays. Even though the rhythm section breaks the time up, there’s a basic pulse coming from inside the tune. That’s the pulse the musicians have to play.”

      There is a head played by horns in unison.

      It’s helpful to focus on one instrument at a time.
    guide: |-
      0:43 | Dolphy (alto saxophone) solo
      3:11 | Hubbard (trumpet) solo
      5:44 | Hutcherson (vibraphone), Davis (bass), and Williams (drums)
      8:15 | Davis (bass) solo, no accompaniment
      9:59 | Dolphy and Hubbard trade short licks over Williams's drum solo
      11:26 | Head out
- heading: Cecil Taylor
  artists:
  - name: Cecil Taylor
    born: 1929
    died: 2018
  prose: |-
    Background in classical music.

    Had gig at the Five Spot in 1956 for 6 weeks.

    Incredible virtuosity. Known for his flurries of notes and his freely improvised solo concerts.

    Has recorded at least a couple drum + piano duets. One with Max Roach, another with Tony Williams.

    Known (in part) for his entirely “free” improvisations on solo piano during concerts which would often last 3+ hours.

    Thought about music in “Unit Structures.”
  tracks:
  - title: Jitney No. 2
    performer: Cecil Taylor
    recorded: July 2, 1974
    album: Silent Tongues
    personnel:
    - [Cecil Taylor, piano]
    spotify: 3PylUt7686g1K1VssYfi8K
    intro: |-
      Recorded at the Montreux Jazz Festival in Switzerland.

      Completely free-form improvisation.

      Staccato attacks. No form. Bass/Treble conversations.

      Does seem to play off a theme, most clearly audible around 0:04, 1:06, and very end.
- heading: Charles Mingus
  artists:
  - name: Charles Mingus
    born: 1922
    died: 1979
  prose: ""
  tracks:
  - title: Don't Be Afraid, the Clown's Afraid Too
    performer: Charles Mingus
    composer: Charles Mingus
    arranger: Sy Johnson
    recorded: October 1, 1971
    album: Let My Children Hear Music
    personnel:
    - [Charles Mingus, bass]
    - [Lonnie Hillyer, trumpet]
    - [Snooky Young, trumpet]
    - [Joe Wilder, trumpet]
    - [Julius Watkins, French horn]
    - [Charles McPherson, alto saxophone]
    - [Bobby Jones, saxophone]
    - [James Moody, saxophone]
    - [Sir Roland Hanna, piano]
    - [Dannie Richmond, drums]
    spotify: 6pCQ5u1P1GGOXllx6H4FhY
    intro: |-
      “To even attempt to describe this music would be foolish, so it’s best to just sit back and let the world of Charles Mingus envelop you.” [(Sultanof, p.147)](bibliography.html#sultanof-2017)

      Liner notes to *Let My Children Hear Music* :
      <http://mingusmingusmingus.com/mingus/what-is-a-jazz-composer>
    guide: |-
      0:00 | Intro, animal noises
      0:20 | Trumpet entrance (probably Snooky Young)
      0:51 | A1, trombone and tuba enter with composed melody
      1:11 | A2, flute and other brass enter with counter line
      1:31 | A3, third counter line is added
      1:53 | B, circus-like theme in 3/4
      2:12 | C, still in 3/4
      2:23 | Back to 4/4
      2:34 | A4
      2:54 | Saxophones solo (tenor, right channel, and alto, left channel)
- heading: Sun Ra
  artists:
  - name: Sun Ra
    born: 1914
    died: 1993
  prose: |-
    Born: Herman Poole Blount.

    Legal name: Le Sony’r Ra.

    From Alabama. He was a prodigious pianist and musician as a child.

    He claimed he was an alien from Saturn. He had a “vision” when in college in which he was abducted by aliens that told him to stop attending college and to play music.

    “Ra” comes from the Egyptian god of the sun. Sun Ra was obsessed with Egyptian occultism. Considered part of an aesthetic/philosophy called Afrofuturism, which may also include Deltron 3030 and the superhero Black Panther.

    The Arkestra continues to perform without Sun Ra to this day.
  tracks:
  - title: Call for All Demons
    performer: Sun Ra and His Arkestra
    composer: Sun Ra
    recorded: July 12, 1956
    album: Jazz by Sun Ra
    personnel:
    - [Sun Ra, 'piano, organ']
    - [Art Hoyle, 'trumpet, bells']
    - [Dave Young, trumpet]
    - [Julian Priester, 'trombone, chimes']
    - [James Scales, alto saxophone]
    - [John Gilmore, 'tenor saxophone, woodblocks']
    - [Pat Patrick, 'baritone saxophone, bells']
    - [Richard Evans, bass]
    - [Wilburn Green, 'electric bass, tambourine']
    - [Robert Barry, 'drums, bells']
    - [Jim Herndon, timpani]
    spotify: 0WwkqtK9ieuYOuCn209EYo
    intro: |-
      "The titles [of Ra's compositions] were meant to serve as guides to the musicians as to what should be played, but he seldom elaborated on the meaning or gave the musicians more specific instructions. 'Each song and album title had a message in it,' Alton Abraham said... '"Call for all Demons," for example, was a warning about the future.'" ([Szwed, Space is the Place: The Lives and Times of Sun Ra](https://books.google.com/books?id=JtUIgpLqh4QC&pg=PT817&dq=sun+ra+call+for+all+demons))
    guide: |-
      0:00 | Groove established
      0:10 | Ra begins improvising. Listen for thick cluster chords and disjunct melodies.
      0:48 | Horns enter with melody. Tambourine also joins.
      1:29 | Gilmore (tenor saxophone) solo over F blues
      1:58 | Young (trumpet) solo
      2:30 | Scales (alto saxophone) solo
      2:59 | Tympani solo by Jim Herndon
      3:14 | Ra (piano) solo
      3:29 | Return to opening groove
      3:34 | Horns (with tambourine) enter and play melody.
      4:15 | Back to groove
      4:21 | Horns enter with new melody, played twice.
      4:48 | Collective improvisation and diminuendo to final chord.
---
