---
title: Saxophonists
description: A survey of contemporary jazz saxophonists and their contributions to modern jazz.
order: 32
media:
  donny-mccaslin:
    youtube: a-cL1a_i3dU
    title: 'Donny McCaslin video'
intro: |-
  In this unit, we explore several prominent contemporary saxophonists. These musicians emerged in the late 20th or early 21st century, playing in contexts as diverse as the last Miles Davis band, contemporary funk and fusion ensembles, acoustic post-bop settings, and even collaborations with pop and rock artists like David Bowie. Their careers and styles exemplify the breadth and depth of today’s jazz saxophone tradition.
sections:
- heading: Kenny Garrett
  artists:
  - name: Kenny Garrett
    born: 1960
  prose: |-
    Read the [Grove Music Online article about Kenny Garrett](https://libproxy.library.unt.edu:2147/10.1093/gmo/9781561592630.article.A2228299) . Garrett, an alto saxophonist and flutist, started young, playing in the Duke Ellington ghost band as a teenager. He worked with the Jazz Messengers and joined Miles Davis’s final band (1986-1991). Miles praised Garrett as developing a unique style rather than just copying past influences.

    Garrett didn't study formally at a university, yet he received an honorary doctorate from Berklee in 2011. His improvisations feature highly scalar lines, wide range usage, and a blend of funky, bluesy edges with Coltrane- and Tyner-inspired harmony.
  tracks:
  - title: J. Mac
    performer: Kenny Garrett Quartet
    composer: Kenny Garrett
    recorded: '2011'
    album: Seeds from the Underground
    personnel:
    - [Kenny Garrett, alto saxophone]
    - [Benito Gonzalez, piano]
    - [Nat Reeves, bass]
    - [Ronald Bruner, drums]
    spotify: 4F2GVbpLBE0G7dPIZMp0tC
    guide: |-
      0:00 | Head in
      0:48 | Garrett (as) solo
      3:37 | Gonzalez (p) solo
      5:21 | Head out
  notes: |-
    - This track evokes the Coltrane/Tyner tradition. The rhythmic figure and fourth-based voicings resemble McCoy Tyner’s style (see "Passion Dance").
    - Garrett uses pentatonic scales and plays “outside” the harmony, then resolves back to the home key (B♭ minor).
- heading: Joshua Redman
  artists:
  - name: Joshua Redman
    born: 1969
  prose: |-
    Read the Grove Music Online article about Joshua Redman. Son of Dewey Redman, Joshua graduated from Harvard in social studies and won the 1991 Thelonious Monk Competition. He declined Yale Law School to pursue music. He led popular quartets and founded the SFJAZZ Collective. Redman’s style adapts to different settings, from complex rhythmic unisons to soft, melodious phrasing.

    Check out his 1994 album *MoodSwing* (with Mehldau, McBride, Blade) for an example of his accessible yet sophisticated style. Later, he explored jazz-funk with the Elastic Band.
  tracks:
  - title: Put it in Your Pocket
    performer: Joshua Redman Elastic Band
    composer: Joshua Redman
    recorded: '2005'
    album: Momentum
    personnel:
    - [Joshua Redman, 'tenor saxophone, keyboards']
    - [Sam Yahel, 'organ, keyboards']
    - [Peter Bernstein, guitar]
    - [Nicholas Payton, trumpet]
    spotify: 2eH9fbAlUoeEoW3VIC3Uct
    intro: |-
      Form: AABACA
    guide: |-
      0:00 | Drum roll into groove, Rhodes, organ, bass
      0:25 | A theme (tp & ts in unison)
      0:50 | B
      1:02 | A
      1:13 | C
      1:26 | A'
      1:40 | Payton (tp) solo, chromatic “out” notes
      2:02 | Overdubbed trumpets support the solo
      2:51 | Redman (ts) solo, quoting Payton’s ending phrase
      3:18 | Overdubbed sax lines support Redman’s solo
      4:04 | Payton & Redman trade
      5:03 | A' head out
      5:15 | Vamp continues
      5:27 | Organ lick repeated
      6:29 | Rhodes & organ drop out
  notes: |-
    - Jazz-funk is now an established part of the jazz landscape. This track shows Redman’s funk side.
    - Sam Yahel often collaborates with Redman, notable on organ and piano.
    - Nicholas Payton is known for his provocative writings on jazz; he was also in SFJAZZ Collective.
    - Bonus: Watch Yo-Yo Ma and Redman cameo in the Arthur cartoon at the provided link.
- heading: Chris Potter
  artists:
  - name: Chris Potter
    born: 1971
  prose: |-
    Read the [JazzTimes article](https://jazztimes.com/features/profiles/chris-potter-master-of-paradox/) about Chris Potter. A child prodigy, Potter attended the New School and Manhattan School of Music. He frequently tops the *DownBeat* polls for Best Tenor Saxophonist. He placed 3rd in the same Monk competition that Joshua Redman won.

    Known for remarkable technique and creativity, Potter recorded an 11-minute unaccompanied version of "All the Things You Are" that has become a cult favorite. He has worked with Maria Schneider, receiving Grammy nominations for his solos in her orchestra. A collaboration with David Bowie on *Blackstar* introduced him to a wider audience.
  tracks:
  - title: Ilimba
    performer: Chris Potter Quartet
    composer: Chris Potter
    recorded: June 2016
    album: The Dreamer is the Dream
    personnel:
    - [Chris Potter, 'tenor saxophone, ilimba']
    - [David Virelles, piano]
    - [Joe Martin, bass]
    - [Marcus Gilmore, 'drums, percussion']
    spotify: 6HkuGt2ZjfxYkGMCXK43BA
    intro: |-
      ...
    guide: |-
      0:00 | Potter plays ilimba (thumb piano), unaccompanied
      0:36 | Groove established with drums
      0:59 | Piano enters with open intervals
      1:23 | Potter (ts) melody
      2:37 | Potter solo
      5:04 | Virelles (p) solo
      7:05 | Composed hits return
      7:19 | Gilmore (d) solo
      9:07 | Head out
  notes: |-
    - Ilimba is a thumb piano similar to an mbira.
- heading: Donny McCaslin
  artists:
  - name: Donny McCaslin
    born: 1966
  prose: |-
    Donny McCaslin grew up in Santa Cruz, CA, attended Berklee on full scholarship, and later joined fusion band Steps Ahead. He has been a key member of the Maria Schneider Jazz Orchestra, receiving Grammy nominations for his solos. Schneider introduced him to David Bowie, leading to McCaslin’s involvement in Bowie’s final album *Blackstar* .

    This brought McCaslin to a new level of fame. In the 2017 Google talk linked below, watch first 9 minutes for his early career story and at 17:24 for the Bowie collaboration.

    @video(donny-mccaslin)
  tracks:
  - title: Bright Abyss
    performer: Donny McCaslin & Ensemble
    composer: Donny McCaslin
    recorded: April 4-6, 2016
    album: Beyond Now
    personnel:
    - [Donny McCaslin, tenor saxophone]
    - [Jason Lindner, keyboards]
    - [David Binney, synthesizer]
    - [Nate Wood, guitar]
    - [Tim Lefebvre, bass]
    - [Mark Guiliana, drums]
    spotify: 2hNdWIQgBP0zEZSGEAj2G6
    intro: |-
      6:15 –
    guide: |-
      0:00 | Intro groove, synth noises
      0:29 | A melody
      1:36 | Anthem-style theme
      3:00 | McCaslin solo builds slowly
      4:55 | Distorted keys, vamp for drums
      7:09 | Anthem theme returns
      7:38 | Vocals added for anthem-like climax
  notes: |-
    - Same band as Bowie’s *Blackstar* sessions.
- heading: Melissa Aldana
  artists:
  - name: Melissa Aldana
    born: 1988
  prose: |-
    Melissa Aldana, from Santiago, Chile, won the 2013 Thelonious Monk saxophone competition at 24, the first woman and first South American to do so in a non-vocal category. She studied at Berklee after growing up learning saxophone from her father. She performed at UNT in 2018.

    Her 2019 album *Visions* was inspired by Frida Kahlo and includes mostly originals except one standard, “Never Let Me Go.”
  tracks:
  - title: Never Let Me Go
    performer: Melissa Aldana Quartet
    composer: Jay Livingston
    recorded: September 14 & 15, 2018
    album: Visions
    personnel:
    - [Melissa Aldana, tenor saxophone]
    - [Sam Harris, piano]
    - [Pablo Menares, bass]
    - [Tommy Crane, drums]
    spotify: 4araZQQ2FfuBV1neE1WGJM
    guide: |-
      0:00 | Aldana solo intro, referencing melody
      1:15 | Rhythm section enters, Aldana states head (long form, mixing melody & improv)
      3:17 | Harris (p) solo, intricate voice-leading, shifting in & out of dissonance
      5:19 | Head out
  notes: |-
    - Notice how Aldana seamlessly blends melody and improvisation over a long form.
    - Harris’s piano solo references gospel voicings (4:25) and Monk-like dissonances (5:00).
---
