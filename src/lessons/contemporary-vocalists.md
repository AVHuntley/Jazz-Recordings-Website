---
title: Vocalists
description: A survey of contemporary jazz vocalists.
order: 34
media:
  kurt-elling:
    youtube: TVqb9X8gGtQ
    title: 'Kurt Elling video'
  jazzmeia-horn:
    youtube: EmI-anDxMUc
    title: 'Jazzmeia Horn video'
  jacob-collier:
    youtube: zua831utwMM
    title: 'Jacob Collier video'
intro: |-
  In this final section, we explore some of the most celebrated contemporary jazz vocalists. These singers have garnered critical acclaim, influenced by past legends but forging new paths with original material, daring arrangements, and a blend of jazz tradition and other musical styles.
sections:
- heading: Kurt Elling
  artists:
  - name: Kurt Elling
    born: 1967
  prose: |-
    Kurt Elling is a highly acclaimed vocalist with a long list of awards, including multiple Grammy nominations and a 2009 win. He was voted Best Male Vocalist in the DownBeat Critics Poll for 15 years (2000-2013, 2018-2019). His style integrates scat, vocalese, improvised lyrics, and literary references, drawing comparisons to 1950s poet-comedian Lord Buckley.

    @video(kurt-elling)
  tracks:
  - title: Lush Life
    performer: Kurt Elling & Ensemble
    composer: Billy Strayhorn
    recorded: January 21, 2009
    album: 'Dedicated to You: Kurt Elling Sings the Music of Coltrane and Hartman'
    credits:
    - '**Form:**

      AABC (some versions repeat C for solos; here it''s quite concise)'
    personnel:
    - [Kurt Elling, vocals]
    - [Laurence Hobgood, piano]
    - [Clark Sommers, bass]
    - [Ulysses Owens, drums]
    - [ETHEL (string quartet), '']
    spotify: 3it8w9ABA7e1LJH5c7hfbh
    intro: |-
      ETHEL: Cornelius Dufallo, Mary Rowell (violin); Ralph Farris (viola); Dorothy Lawson (cello);

      Form: AABC (on other versions, C is repeated for solos. A and B never repeat.)
    guide: |-
      0:00 | A: Vocal & piano rubato
      0:33 | A: Reharmonized chords
      1:07 | B
      1:50 | C, band enters, tempo established
      2:49 | D? Extension
      3:27 | Elling holds high note, band lays out briefly
      3:36 | Band reenters, final phrase
      3:59 | Vocal cadenza, strings + piano end
  notes: |-
    - "Lush Life" by Billy Strayhorn is a sophisticated standard. Famous versions include Coltrane/Hartman’s.
    - ETHEL is an amplified, sometimes improvising string quartet.
- heading: Gregory Porter
  artists:
  - name: Gregory Porter
    born: 1971
  prose: |-
    Gregory Porter is a Grammy-winning vocalist and composer. He was a collegiate football player before focusing on music. He wrote a musical "Nat King Cole and Me." He’s often dominated DownBeat polls for Male Vocalist when Elling hasn't. Porter writes much of his own material and has a strong crossover appeal.
  tracks:
  - title: Be Good (Lion’s Song)
    performer: Gregory Porter & Ensemble
    composer: Gregory Porter
    recorded: June 2011
    album: Be Good
    credits:
    - '**Form:**

      Verse-Chorus structure'
    personnel:
    - [Gregory Porter, vocals]
    - [Keyon Harrold, trumpet]
    - [Yosuke Sato, alto saxophone]
    - [Tivon Pennicott, tenor saxophone]
    - [Chip Crawford, piano]
    - [Aaron James, bass]
    - [Emanuel Harrold, drums]
    spotify: 7Joyne3CVPOx7OusuDvnqy
    intro: |-
      Form: It's a two-part structure. Perhaps just AB. I've labeled it Verse-Chorus below, since that's what it feels like to me.
    guide: |-
      0:00 | Intro
      0:16 | Verse 1
      0:48 | Chorus (partido alto groove)
      1:21 | Verse 2
      1:52 | Chorus
      2:24 | Sax solo
      2:54 | Sax solo over chorus
      3:24 | Verse 3
      3:55 | Chorus
      4:29 | Verse 4
      4:59 | Chorus
      5:31 | Vamp & fade
  notes: |-
    - Watch the official music video for a visual narrative.
    - The chorus groove is a sped-up partido alto pattern from Brazilian funk music.
- heading: Esperanza Spalding
  artists:
  - name: Esperanza Spalding
    born: 1984
  prose: |-
    Esperanza Spalding is a vocalist, bassist, composer, and a prodigious talent. She attended Berklee and became the youngest instructor in Berklee’s history at age 20. She’s won multiple Grammys, including a historic Best New Artist win in 2011 (the first jazz artist to do so).

    Drummer Terri Lyne Carrington once described her as having virtuosic bass technique, vocal acrobatics, challenging compositions, poetic lyrics, and commercial appeal.
  tracks:
  - title: I Know You Know
    performer: Esperanza Spalding Ensemble
    composer: Esperanza Spalding
    recorded: '2008'
    album: Esperanza
    credits:
    - '**Form:**

      Verse-Chorus-Bridge form with a Brazilian partido alto groove in the chorus.'
    personnel:
    - [Esperanza Spalding, 'bass, vocals']
    - [Leo Genovese, piano]
    - [Otis Brown, drums]
    - [Jamey Haddad, percussion]
    spotify: 3afeZ81GDy59tATovKyH6Z
    guide: |-
      0:00 | Intro bass line
      0:23 | Verse 1
      0:44 | Chorus (fast partido alto)
      0:56 | Verse 2
      1:17 | Chorus
      1:38 | Bridge
      1:54 | Piano solo over static harmony
      2:15 | Chorus
      2:36 | Back to intro groove, fade
  notes: |-
    - Watch a live performance on YouTube to see Spalding’s remarkable stage presence and musicianship.
    - The chorus groove is borrowed from a Brazilian funk known as partido alto, sped up considerably.
- heading: Cécile McLorin Salvant
  artists:
  - name: Cécile McLorin Salvant
    born: 1989
  prose: |-
    Cécile McLorin Salvant, from Miami, won the 2010 Thelonious Monk competition and has dominated the DownBeat Critics Poll as Best Female Vocalist since 2014. She’s won multiple Grammys for Best Vocal Jazz Album. Salvant’s work blends historical reference and modern interpretation, with a brilliant command of phrasing and storytelling.
  tracks:
  - title: I Didn’t Know What Time It Was
    performer: Cécile McLorin Salvant & Ensemble
    composer: Richard Rodgers (music), Lorenz Hart (lyrics)
    recorded: Sept 9-11, 2016
    album: Dreams and Daggers
    credits:
    - '**Form:**

      AABA'''
    personnel:
    - [Cécile McLorin Salvant, vocals]
    - [Aaron Diehl, piano]
    - [Paul Sikivie, bass]
    - [Lawrence Leathers, drums]
    spotify: 4h42APp9n0wq4wIZHgcPPE
    intro: |-
      Form: AABA'
    guide: |-
      0:00 | Piano intro
      0:15 | A
      0:32 | A
      0:48 | B
      1:05 | A'
      1:29 | Salvant “solos” by subtly altering melody & phrasing
      2:38 | Bass (Sikivie) solo
      4:54 | Head out, Salvant returns to lyrics, still improvisational
      6:01 | Ending
  notes: |-
    - Recorded live at the Village Vanguard.
    - Watch a video of a live performance of this arrangement to see her approach.
- heading: Jazzmeia Horn
  artists:
  - name: Jazzmeia Horn
    born: 1991
  prose: |-
    Jazzmeia Horn, from Dallas, won the 2013 Sarah Vaughan Competition and the 2015 Thelonious Monk Competition. Her 2017 debut album, *A Social Call* , garnered widespread acclaim. Influenced by her upbringing singing in church, Horn brings a powerful, soulful presence to jazz standards and originals.

    @video(jazzmeia-horn)
  tracks:
  - title: I Remember You
    performer: Jazzmeia Horn & Ensemble
    composer: Victor Schertzinger (music), Johnny Mercer (lyrics)
    recorded: '2017'
    album: A Social Call
    credits:
    - '**Form:**

      AABA'
    personnel:
    - [Jazzmeia Horn, vocals]
    - [Victor Gould, piano]
    - [Bennie Williams, bass]
    - [Jerome Jennings, drums]
    spotify: 6qnpyuyEKEmcM2R0xbLjzm
    intro: |-
      *Form: AABA*
    guide: |-
      0:03 | AA, head in
      0:17 | B
      0:24 | A
      0:35 | Horn scats
      1:08 | Drum solo (Jennings)
      1:35 | AABA head out
  notes: |-
    - Watch Horn perform this live to appreciate her joyful interpretation.
- heading: Jacob Collier
  artists:
  - name: Jacob Collier
    born: 1994
  prose: |-
    Jacob Collier is a vocalist, multi-instrumentalist, composer, and arranger from London. Quincy Jones discovered him from his early YouTube videos. He’s won multiple Grammys for his elaborate, cutting-edge arrangements. Collier’s mastery extends to all instruments, digital audio workstations, and intricate vocal harmonies.

    @video(jacob-collier)
  tracks:
  - title: Flintstones
    performer: Jacob Collier (solo production)
    composer: Hoyt Curtin
    recorded: '2016'
    album: In My Room
    credits:
    - '**Form:**

      Based on the 16-bar AABA form of the original cartoon theme, though Collier transforms it with layers of harmony and groove.'
    spotify: 1wF1K0thTrzCveF604G8po
    intro: |-
      Jacob Collier (everything)

      Form: Well, the original is 16-bar AABA. I'll let you figure out how Collier does it.
  notes: |-
    - Originally from the classic TV theme, Collier’s version is a multi-tracked vocal extravaganza.
    - This arrangement won Collier a Grammy for Best Arrangement.
---
