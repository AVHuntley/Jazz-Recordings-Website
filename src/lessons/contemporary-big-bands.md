---
title: Contemporary Big Bands
description: A history of contemporary big bands and their recordings.
order: 29
media:
  holman-nea:
    youtube: 62PfNNANmKQ
    title: Bill Holman, NEA Jazz Masters video
  schneider-orchestra:
    youtube: Vl7ngJ6Tnnk
    params: list=PLkDTD5EGiVtTMj_vOy9SkeIuTt7tQkgRX
    title: Maria Schneider Orchestra
  nash-presidential-suite:
    youtube: -SR9jlnSf7g
    title: 'Ted Nash: Presidential Suite'
  nash-live:
    youtube: xEWCluBbLQY
    title: Presidential Suite, live performance
  ofarrill-jazz-night:
    youtube: nB4TETXF3I4
    title: 'Arturo O’Farrill on Jazz Night in America'
  ofarrill-live:
    youtube: A6VwgJQlbO8
    params: list=PLNqtYbDgMtPOWKvjrPxa0fCO8aKSQmXsZ
    title: Afro Latin Jazz Orchestra, live December 18, 2014
  kamasi-live:
    youtube: 0YbPSIXQ4q4
    title: Kamasi Washington, live August 2015
  mintzer-havin-some-fun:
    youtube: 8Tx7Mi6Uenw
    title: Bob Mintzer Big Band, Havin’ Some Fun
intro: |-
  In the current decade, the big band remains a comparatively minor part of the jazz world. But that doesn't mean it has gone extinct. Though only a few big bands continue to tour the way they did in the 1930s and 40s, many communities maintain **rehearsal bands** . Several prominent players and arrangers still occasionally record big band albums. And some bands even continue the legacy of their deceased figurehead in the form of a **ghost band** .

  The [Grammy Award for Best Large Jazz Ensemble Album](https://en.wikipedia.org/wiki/Grammy_Award_for_Best_Large_Jazz_Ensemble_Album) is awarded every year. The list of nominees for and recipients of this award is one way I determined what bands to feature in this lesson and [the next]($WIKI_REFERENCE$/pages/5-dot-2-contemporary-big-bands-pt-2 "5.2 - Contemporary Big Bands Pt. 2") .

  ## Vocabulary
  **Rehearsal band**
  - a big band that meets regularly, often weekly at the same location, to play for fun and practice, if not always profit. They rarely tour, though many sometimes record. Their personnel is not always consistent.

  **Ghost band**
  - a band that plays under the name of a deceased leader. Several bands from the swing era exist in this form, including the [Glenn Miller Orchestra](https://glennmillerorchestra.com/) , the [Count Basie Orchestra](https://www.thecountbasieorchestra.com/) , the [Ellington Orchestra](http://www.dukeellington.com/orchestra.html) , and the [Mingus Big Band](https://www.charlesmingus.com/mingus-big-band) .
sections:
- heading: Mingus Big Band
  prose: |-
    The Mingus Big Band is one of a few currently active ensembles specializing in the music of Charles Mingus. The other two bands are the [Mingus Orchestra](https://www.charlesmingus.com/orchestra) and the [Mingus Dynasty](https://www.charlesmingus.com/dynasty) . The three bands perform on [Mondays at the Jazz Standard](https://www.jazzstandard.com/programs/#Mingus) , a venue in NYC. Each band was formed and organized by Sue Mingus, Charles's widow. She put together the Mingus Big Band in 1997.

    The popularity of the Mingus Big Band was cemented when their 2010 album *Live at Jazz Standard* won the Grammy for Best Large Jazz Ensemble album.

    Its ever-evolving personnel list has included such greats as Randy Brecker, Donny McCaslin, Gary Smulyan, Orrin Evans, Kenny Drew, Jr., and Jeff "Tain" Watts.
  tracks:
  - title: Jump Monk
    performer: Mingus Big Band
    composer: Charles Mingus
    recorded: July 1994
    album: Gunslinging Birds
    personnel:
    - [Randy Brecker, trumpet]
    - [Philip Harper, trumpet]
    - [Ryan Kisor, trumpet]
    - [Jamal Haynes, trombone]
    - [Earl McIntyre, trombone]
    - [Frank "Ku-umba" Lacy, trombone]
    - [David Lee Jones, alto saxophone]
    - [Steve Slagle, alto saxophone]
    - [Craig Handy, tenor saxophone]
    - [Chris Potter, tenor saxophone]
    - [John Stubblefield, tenor saxophone]
    - [Gary Smulyan, baritone saxophone]
    - ['Kenny Drew, Jr.', piano]
    - [Andy McKee, bass]
    - [Adam Cruz, drums]
    spotify: 3AfmpoqFIgbLO9HujfTYnn
    guide:
    - ['0:00', 'Intro, Andy McKee (b) and John Stubblefield (ts). Stubblefield quotes “Take the Coltrane.”']
    - ['0:14', Drums and 2nd sax enter]
    - ['0:28', Kenny Drew (p) and Frank Lacy (tb) enter]
    - ['0:42', Head in]
    - ['1:25', Frank Lacy (tb) solo]
    - ['3:17', 'Kenny Drew, Jr. (p) solo, no accompaniment']
    - ['4:05', 'Drew (p) plays a montuno, drums and bass enter']
    - ['4:24', Horn backgrounds added]
    - ['5:06', Head out]
  notes: |-
    - [The first recording of "Jump Monk"](https://open.spotify.com/track/4tN17zX7lKkFQjWnabQ66x?si=eRQL_12uSnWDKLhpHeSUKQ) comes from Mingus's 1955 album *Mingus at the Bohemia* with Max Roach. Even in the opening moments of that original recording, you can hear the collective improvisation that was a hallmark of Mingus's style.
- heading: Gordon Goodwin
  artists:
  - name: Gordon Goodwin
    born: 1954
  prose: |-
    Gordon Goodwin is a Los Angeles-based pianist, saxophonist, composer and arranger who has worked extensively in the film and television industry. He attended Cal State Northridge (CSUN) and produced music for several blockbuster films, including *The Incredibles* . He formed the Big Phat Band in 1999, mostly, as he writes on [his website](http://www.bigphatband.com/about-bpbbio) , "with the intention of recording a record and nothing more. I wanted to document the music I was writing for the big band genre, but no way did I want to hassle with doing live gigs..." A live performance at his alma mater CSUN convinced him to keep the band together.

    In 2015, the Big Phat Band won the Grammy for Best Large Jazz Ensemble Album with *Life in the Bubble* . Prior to that, the band's second album, *XXL* , was nominated for the award, and the composition "Hunting Wabbits" was nominated for Best Instrumental Composition.
  tracks:
  - title: Hunting Wabbits
    performer: Gordon Goodwin’s Big Phat Band
    composer: Gordon Goodwin
    recorded: '2003'
    album: XXL
    personnel:
    - [Wayne Bergeron, trumpet]
    - [Dan Fornero, trumpet]
    - [Stan Martin, trumpet]
    - [Alex Iles, trombone]
    - [Steve Holtman, trombone]
    - [Craig Ware, bass trombone]
    - [Sal Lozano, 'alto saxophone, soprano saxophone, flute, alto flute']
    - [Eric Marienthal, 'alto saxophone, soprano saxophone, flute']
    - [Gordon Goodwin, 'tenor saxophone, piano']
    - [Brian Scanlon, 'tenor saxophone, flute, clarinet']
    - [Jeff Driskill, 'tenor saxophone, flute, clarinet']
    - [Jay Mason, 'baritone saxophone, bass clarinet, flute']
    - [Grant Geissman, guitar]
    - [Richard Shaw, bass]
    - [Peter Erskine, drums]
    - [Luis Conte, percussion]
    spotify: 6O1Wq633TG4EGDPbVZW7YU
    intro: |-
      Bergeron (tp) was in Maynard Ferguson’s band and played trumpet on *The Incredibles* soundtrack.
    guide:
    - ['0:00', 'Saxophones alone, cartoon-like theme']
    - ['0:47', 'Trombones alone, same theme']
    - ['1:26', 'Band enters, same theme']
    - ['2:18', 'Andy Martin (tb) solo, band swings, minor blues']
    - ['3:13', Brian Scanlon (ts) solo]
    - ['4:07', Luis Conte (bongos) solo]
    - ['4:19', Band layers riffs to crescendo]
    - ['5:19', Band and drums trade phrases]
    - ['5:40', 'Outro, cartoon-theme again']
  notes: |-
    - If this recording reminds you of cartoon music, that's no mistake. Goodwin has composed for both *The Incredibles* and Looney Tunes. The title refers to Elmer Fudd’s phrase "Hunting Wabbits."
- heading: Bill Holman
  artists:
  - name: Bill Holman
    born: 1927
  prose: |-
    Bill Holman is a saxophonist, composer, arranger, and bandleader. He attended UCLA, then worked with several renowned musicians, including Stan Kenton, Shorty Rogers, Mel Lewis, Maynard Ferguson, Gerry Mulligan, and Woody Herman. Read the short [Grove Music Online article about Holman](https://libproxy.library.unt.edu:2147/10.1093/gmo/9781561592630.article.J206000) .

    From 1975 he led a big band in the Los Angeles area... After some dozen years Holman finally secured a recording contract for his big band. It became one of the finest such ensembles of the 1990s, and made a widely acclaimed recording of Holman’s creative arrangements of the compositions of Thelonious Monk (1997).

    To date, Holman has received 14 Grammy nominations and won three Grammy Awards. He was named an [NEA Jazz Master](https://www.arts.gov/honors/jazz) . You can read more about him at his [NEA biography page](https://www.arts.gov/honors/jazz/bill-holman) .

    Watch this short video produced to commemorate Holman's becoming an NEA Jazz Master. At [1:28](https://youtu.be/62PfNNANmKQ?t=88) , he talks about participating in the 1950s West Coast Jazz scene, and at 3:10 about forming his own big band.

    @video(holman-nea)
  tracks:
  - title: Bemsha Swing
    performer: Bill Holman Big Band
    composer: Thelonious Monk
    recorded: May 29, 2006
    album: Hommage
    personnel:
    - ['Carl Saunders, Pete DiSiena, Ron Stout, Bob Summers, Roger Ingram', trumpet]
    - ['John Grab, Bob Enevoldsen, Andy Martin', trombone]
    - [Craig Gosnell, bass trombone]
    - ['Lanny Morgan, Bruce Babad', 'alto saxophone, soprano saxophone, flute']
    - ['Pete Christlieb, Ray Herrmann', 'tenor saxophone, other reeds']
    - [Bob Efford, baritone saxophone]
    - [Christian Jacob, piano]
    - [Joel Hamilton, bass]
    - [Kevin Kanner, drums]
    spotify: 2sjWREmviP41o48GILXSRo
    guide:
    - ['0:00', Piano intro]
    - ['0:06', Head in]
    - ['0:32', Saxophones play counter-line]
    - ['0:57', Christian Jacob (p) solo]
    - ['2:15', '(tp) solo, with backgrounds']
    - ['3:33', Ensemble passage]
    - ['4:01', Ensemble trades with Kevin Kanner (d)]
    - ['4:49', (ts) solo]
    - ['6:08', Head out]
    - ['7:28', Outro]
  notes: |-
    - This arrangement was first recorded in 1997 and released on the album *Brilliant Corners* (also a famous Monk album title). Holman recorded this arrangement again for *Hommage* .
- heading: Maria Schneider
  artists:
  - name: Maria Schneider
    born: 1960
  prose: |-
    The Grove Music Online article on Schneider is outdated, so read [this Britannica article](https://www.britannica.com/biography/Maria-Schneider-American-musician) . She studied composition from a young age, earned degrees at University of Minnesota and Eastman, studied with Bob Brookmeyer, and worked as a copyist for Gil Evans. She formed The Maria Schneider Jazz Orchestra in 1992.

    She has been a vocal opponent of online music streaming services, testifying before the US Congress. She was an early adopter of crowd funding, releasing albums through [ArtistShare](https://www.artistshare.com/) . Her album *Concert in the Garden* (2004) won a GRAMMY with Internet-only sales, and blazed the crowd-funding trail.

    Her album *The Thompson Fields* won the 2015 Grammy for Best Large Jazz Ensemble Album.

    @video(schneider-orchestra)
  tracks:
  - title: Walking by Flashlight
    performer: Maria Schneider Orchestra
    composer: Maria Schneider
    recorded: August 2014
    album: The Thompson Fields
    personnel:
    - [Dave Pietro, 'alto saxophone, clarinet, piccolo, flute, alto flute, bass flute']
    - [Steve Wilson, 'alto saxophone, soprano saxophone, clarinet, flute, alto flute']
    - [Rich Perry, 'tenor saxophone, flute']
    - [Donny McCaslin, 'tenor saxophone, clarinet']
    - [Scott Robinson, 'baritone saxophone, clarinet, bass clarinet']
    - ['Greg Gisbert, Augie Haas, Tony Kadleck, Mike Rodriguez', 'trumpet, flugelhorn']
    - ['Marshall Gilkes, Ryan Keberle, Keith O''Quinn', trombone]
    - [George Flynn, 'bass trombone, contrabass trombone']
    - [Frank Kimbrough, piano]
    - [Gary Versace, accordion]
    - [Lage Lund, guitar]
    - [Jay Anderson, bass]
    - [Clarence Penn, drums]
    spotify: 0CGn33qDCIANhMkyONBSWj
    guide:
    - ['0:00', 'Piano chords, high note on accordion sets mood']
    - ['0:09', 'A, played by Scott Robinson on alto clarinet']
    - ['0:18', Bass enters]
    - ['0:43', 'A again, with horn background pad']
    - ['1:19', B]
    - ['1:54', 'A, piano leads, accordion & clarinet in background']
    - ['2:29', Robinson (alto clarinet) solo]
    - ['3:35', Kimbrough (p) solo]
    - ['4:05', Robinson solos again]
    - ['4:30', 'Melody ends, piano, accordion, and bass fade']
  notes: |-
    - The album is based on poetry by Ted Kooser. "Walking By Flashlight" is inspired by his poem about November mornings.
    - Schneider offers charts and play-along tracks via ArtistShare.
    - Donny McCaslin received Grammy nominations for his work with Schneider.
- heading: Ted Nash with Jazz at Lincoln Center Orchestra
  artists:
  - name: Ted Nash
    born: 1960
  prose: |-
    Jazz at Lincoln Center (JALC) is a not-for-profit in NYC promoting jazz performance and education. Wynton Marsalis is artistic director. They consider jazz "America's music" and a metaphor for democracy.

    JALC maintains the Jazz at Lincoln Center Orchestra (JLCO), featuring 15 top musicians. Read the short entry on Ted Nash [here](https://www.jazz.org/JLCO/ted-nash/) .

    In 2014, Nash recorded an album where each composition was inspired by the pitches and cadence of famous speeches. The album, *Presidential Suite: Eight Variations on Freedom* , won the 2017 Grammy for Best Large Jazz Ensemble Album. “Spoken at Midnight” won the 2017 Grammy for Best Instrumental Composition.

    @video(nash-presidential-suite)
  tracks:
  - title: Spoken at Midnight (Jawaharlal Nehru)
    performer: Jazz at Lincoln Center Orchestra
    composer: Ted Nash
    recorded: September 14, 2014
    album: 'Presidential Suite: Eight Variations on Freedom'
    personnel:
    - ['Ryan Kisor, Kenny Rampton, Marcus Printup, Greg Gisbert', trumpet]
    - ['Vincent Gardner, Chris Crenshaw, Elliot Mason', trombone]
    - [Ted Nash, 'alto saxophone, soprano saxophone']
    - [Sherman Irby, 'alto saxophone, flute, alto flute']
    - [Charles Pillow, 'alto saxophone, flute, alto flute, clarinet, soprano saxophone']
    - [Victor Goines, 'tenor saxophone, clarinet, bass clarinet, alto flute']
    - [Walter Blanding, 'tenor saxophone, soprano saxophone, clarinet']
    - [Paul Nedzela, 'baritone saxophone, bass clarinet']
    - [Dan Nimmer, piano]
    - [Carlos Henriquez, 'bass, electric bass']
    - [Ali Jackson, 'drums, percussion']
    spotify: 3hRFWwZW2YMifBoDtX9oj1
    guide:
    - ['0:00', 'Bass intro in 7, then b-cl, then fl and p']
    - ['0:22', 'Melody, muted tp (and fl?)']
    - ['1:29', 'Groove interlude, bass and drums']
    - ['1:40', Nash (sop) solo]
    - ['3:49', 'Groove interlude again, w/ piano LH']
    - ['4:01', 'Melody again, unmuted brass']
    - ['5:11', 'Groove with bass, chekeré, piano']
  notes: |-
    - Here's a video of a live performance of this album’s material. Ted Nash introduces the band at 0:53. The reading of “Tryst with Destiny” and “Spoken at Midnight” start at 11:38.

    @video(nash-live)

    - Jawaharlal Nehru was the first prime minister of India (1947-1964). His speech “Tryst with Destiny” was delivered near midnight, August 14, 1947, the night before India gained independence from Britain.
- heading: Arturo O’Farrill & the Afro Latin Jazz Orchestra
  artists:
  - name: Arturo O’Farrill
    born: 1960
  prose: |-
    Arturo O'Farrill is a pianist, composer, and educator who directs the Afro Latin Jazz Alliance, formerly the Afro Latin Jazz Orchestra. Read a [short biography on his website](http://www.arturoofarrill.com/about-arturo) . He is the son of [Chico O’Farrill](https://libproxy.library.unt.edu:2147/10.1093/gmo/9781561592630.article.A2242503) (1921-2001), a renowned Cuban musician who composed for Machito and arranged for Dizzy Gillespie and Stan Kenton.

    Chico and Arturo formed the Afro-Cuban Jazz Orchestra in the 1990s. In 2002, this band became a resident orchestra at Jazz at Lincoln Center. In 2007, they split from Jazz at Lincoln Center and formed under a new organization, the Afro Latin Jazz Alliance. As their website explains:

    In early 2007, O’Farrill and the Afro Latin Jazz Orchestra (ALJO) left Jazz at Lincoln Center to pursue the twin goals of developing new audiences for big band Latin jazz and creating a robust educational program for young performers. With the support of prominent leaders from the worlds of jazz and Latin culture, O’Farrill launched ALJA to serve as a non-profit organization that could advance both the performance and educational aspects of this uniquely Pan-American art form.

    Their album *Cuba: The Conversation Continues* is dedicated to the complicated relationship between Cuba and the United States. It was nominated for the 2016 Grammy for Best Large Jazz Ensemble album. Watch this video from *Jazz Night in America* about the project (watch at least the first five minutes):

    @video(ofarrill-jazz-night)
  tracks:
  - title: Guajira Simple
    performer: Arturo O'Farrill & the Afro Latin Jazz Orchestra
    composer: Arturo O’Farrill
    recorded: December 19, 20 & 21, 2014
    album: 'Cuba: The Conversation Continues'
    personnel:
    - ['Jim Seeley, Adam O''Farrill, Seneca Black', trumpet]
    - [Rafi Malkiel, trombone]
    - ['Rudresh Mahanthappa, Michel Herrera, Ivan Renta, Alejandro Aviles', reeds]
    - ['Arturo O''Farrill, Michele Rosewoman, Alexis Bosch', piano]
    - [Gregg August, bass]
    - [Vince Cherico, drums]
    spotify: 5sSe8lFTyXpHK51g8CKncf
    guide:
    - ['0:00', 'Intro, solo piano']
    - ['1:23', Beginning (where video starts)]
    - ['2:41', 'Clarinet, piano, and trombone all solo at once']
    - ['3:38', Theme]
    - ['4:16', '(ts) solo, only bass and perc at first']
    - ['5:50', Ensemble]
    - ['5:54', 'Rubato, flute and piano and perc.']
    - ['6:40', Piano plays line in octaves to establish time]
    - ['7:00', 'Out of time again, piano hints at intro vamp']
    - ['7:27', 'Drums reenter, band layers']
    - ['8:23', 'Theme, full band']
    - ['8:44', Band and drums trade 2s]
  notes: |-
    - Here's a video of a live performance from December 18, 2014 (the day before recording began):

    @video(ofarrill-live)
- heading: Kamasi Washington
  artists:
  - name: Kamasi Washington
    born: 1981
  prose: |-
    Kamasi Washington is a saxophonist from Los Angeles. He studied ethnomusicology at UCLA. His 2015 album, *The Epic* , was a breakout hit, leading Washington to collaborate with Wayne Shorter, Herbie Hancock, Lauryn Hill, Snoop Dogg, George Duke, Chaka Khan, and Thundercat. He also played saxophone on Kendrick Lamar’s *To Pimp a Butterfly* .

    While the band playing on *The Epic* is not a typical big band, it is certainly big, including choir, strings, two drummers, two bass players, and multiple horns. As Jeff Sultanof notes, “This composition shows how ambitious composer/arrangers had become in the twenty-first century…”
  tracks:
  - title: Re Run
    performer: Kamasi Washington & ensemble
    composer: Kamasi Washington
    recorded: March 10, 2015
    album: The Epic
    credits:
    - 'Form: AABA'
    personnel:
    - [Igmar Thomas, trumpet]
    - [Ryan Porter, trombone]
    - [Kamasi Washington, tenor saxophone]
    - [Cameron Graves, piano]
    - ['Neel Hammond, Tylana Renga Enomoto, Paul Cartwright, Jennifer Simone, Lucia Micarelli', violin]
    - ['Molly Rodgers, Andrea Whitt', viola]
    - ['Artyom Manukyan, Ginger Murphy', cello]
    - [Miles Mosley, bass]
    - [Tony Austin, drums]
    - [Leon Mobley, percussion]
    - ['Patrice Quinn, Dawn Norfleet, Thalma de Freitas, Malya Sykes, Gina Manziello, Patrice Quinn, Natasha F Agrama, Dwight Trible, Steven Wayne, Taylor Graves, Charles Jones, Jason Morales, Dexter Story, Cameron Graves, Tracy Carter', vocals (choir)]
    spotify: 7H0Ie2lJE0wXT5TfjznEGT
    intro: |-
      Form: AABA
    guide:
    - ['0:00', Strings and choir introduction]
    - ['0:39', Bass begins groove]
    - ['0:46', Drums and piano enter]
    - ['1:04', 'Theme, (ts) and (tb), choir and strings backgrounds']
    - ['2:13', Ryan Porter (tb) solo]
    - ['2:47', Synth background at bridge of (tb) solo]
    - ['3:21', Washington (ts) solo]
    - ['5:57', 'Washington repeats riff, harmony becomes static. Groove.']
    - ['6:29', Theme returns]
    - ['7:35', Tag ending]
    - ['7:55', Last chord]
  notes: |-
    - Here's a live performance from August 2015 (“Re-Run” starts at 1:15:12):

    @video(kamasi-live)

    - The album also contains “Re Run Home,” a reprise of this tune, released as a single.
- heading: Bob Mintzer
  artists:
  - name: Bob Mintzer
    born: 1953
  prose: |-
    Bob Mintzer is a saxophonist, composer/arranger, and educator. Read his detailed [biography](http://www.bobmintzer.com/press.php) . He has played with the Brecker brothers, Jaco Pastorius, Tito Puente, the Thad Jones/Mel Lewis band, and many others. He attended the Manhattan School of Music and taught there for 25 years.

    He formed his first big band in 1983. He is also a longstanding member of the fusion group The Yellowjackets. He currently teaches at the USC Thornton School of Music.

    His album *Homage to Basie* won the Grammy for Best Large Jazz Ensemble in 2002. That album included "Havin’ Some Fun," which he revisited on *All L.A. Band* (nominated for a Grammy in 2017).

    @video(mintzer-havin-some-fun)
  tracks:
  - title: Havin’ Some Fun
    performer: Bob Mintzer Big Band
    composer: Bob Mintzer
    recorded: '2016'
    album: All L.A. Band
    personnel:
    - [Bob Mintzer, tenor saxophone]
    - ['Wayne Bergeron, James Blackwell, John Thomas, Chad Willis, Michael Stever', trumpet]
    - [Bob Sheppard, saxophone]
    - [Adam Schroeder, baritone saxophone]
    - ['Bob McChesney, Erik Hughes, Julianne Gralle', trombone]
    - [Craig Gosnell, bass trombone]
    - [Russ Ferrante, piano]
    - [Larry Koonse, guitar]
    - [Edwin Livingston, bass]
    - [Peter Erskine, drums]
    - [Aaron Serfaty, percussion]
    spotify: 1HdiG60WRLq4XNjEf736mm
    guide:
    - ['0:00', Intro]
    - ['0:16', Head in]
    - ['1:29', Send-off]
    - ['1:37', Mintzer (ts) solo]
    - ['2:00', Backgrounds added]
    - ['2:43', Adam Schroeder (bar) solo]
    - ['3:52', Ensemble passage]
    - ['4:01', 'Saxophone soli, trade with full band']
    - ['4:40', Head out]
  notes: |-
    - Many of the musicians on this album are USC faculty, including Mintzer, Sheppard, Erskine, Ferrante, Livingston, Serfaty, Stever, and Thomas. Many others are USC graduates.
    - You can buy sophisticated play-along tracks for this album if you have an iOS device. Such services are common today, as album sales alone are not always lucrative.
- heading: Christian McBride
  artists:
  - name: Christian McBride
    born: 1972
  prose: |-
    Christian McBride is one of the most in-demand bass players of his generation. After studying at Juilliard, he played with Freddie Hubbard, Sonny Rollins, J.J. Johnson, Ray Brown, Milt Jackson, McCoy Tyner, Roy Haynes, Chick Corea, Herbie Hancock, Joshua Redman, Brad Mehldau, and Pat Metheny. Read more on [his website](https://www.christianmcbride.com/bio.html) .

    McBride is also a producer for *Jazz Night in America* and works regularly as an educator, for example with [Jazz House Kids](https://jazzhousekids.org/) .

    In 2012, McBride released *The Good Feeling* which won a Grammy for Best Large Jazz Ensemble album. The 2015 followup, *Bringin' It* , also won a Grammy. Both albums included an arrangement of "Gettin' To It."
  tracks:
  - title: Gettin’ To It
    performer: Christian McBride Big Band
    composer: Christian McBride
    recorded: '2017'
    album: Bringin’ It
    personnel:
    - ['Frank Greene, Freddie Hendrix, Brandon Lee, Nabate Isles', trumpet]
    - ['Michael Dease, Joe McDonough, James Burton', trombone]
    - [Douglas Purviance, bass trombone]
    - [Steve Wilson, 'alto saxophone, soprano saxophone, flute']
    - [Todd Bashore, 'alto saxophone, flute, piccolo']
    - [Ron Blake, 'tenor saxophone, flute']
    - [Dan Pratt, 'tenor saxophone, clarinet']
    - [Carl Maraghi, 'baritone saxophone, bass clarinet']
    - [Xavier Davis, piano]
    - [Rodney Jones, guitar]
    - [Christian McBride, bass]
    - [Quincy Phillips, drums]
    spotify: 6AAUsr4Bq6r9nAgCaebDKo
    guide:
    - ['0:00', Intro]
    - ['0:16', Head in]
    - ['1:18', (tp) solo]
    - ['2:56', (ts) solo]
    - ['4:36', 'New groove established by bari, b-tb, and bass']
    - ['4:54', Rodney Jones (g) solo]
    - ['5:54', 'Head out, but without explicit melody']
  notes: |-
    - Christian McBride visited UNT in 2017. He performed "Gettin' To It" with the One O'Clock Lab Band at 00:28:43 on the UNT archive page.
    - Pianist Xavier Davis is the brother of UNT drum professor Quincy Davis.
- heading: John Daversa Big Band
  artists:
  - name: John Daversa
    born: 1972
  prose: |-
    John Daversa is a trumpeter, composer/arranger, and educator. He studied classical composition at UCLA, then formed his big band. He previously taught at Cal State Northridge and is now Chair of Jazz at the Frost School of Music at the University of Miami. Read [this AllAboutJazz article](https://www.allaboutjazz.com/john-daversa-bursting-out-of-la-john-daversa-by-rj-deluke.php) for an overview of his career.

    He often plays an EVI (electronic valve instrument).

    His 2018 album *American Dreamers: Voices of Hope, Music of Freedom* is explicitly political. All personnel except Daversa were brought to the US as children (dreamers). The album won the 2019 Grammy for Best Large Jazz Ensemble Album, and Daversa won the Grammy for Best Improvised Jazz Solo on “Don’t Fence Me In.”
  tracks:
  - title: Don’t Fence Me In
    performer: John Daversa Big Band feat. DACA "Dreamers"
    composer: Cole Porter
    arranger: John Daversa
    recorded: March-July 2018
    album: 'American Dreamers: Voices of Hope, Music of Freedom'
    credits:
    - 'Personnel: John Daversa (trumpet) + 53 unidentified dreamer musicians'
    spotify: 35LMyikKwcxc9WFhRSmeQj
    guide:
    - ['0:00', Drum solo intro]
    - ['0:05', 'A section: band plays first phrase, voices respond "Don''t Fence Me In!"']
    - ['0:16', “¡No robes mi sueño!” ("Don't steal my dream!")]
    - ['0:33', Bridge]
    - ['0:43', A again]
    - ['0:55', 'Daversa (tp) solo (Grammy-winning solo). Free form, not strictly following tune.']
    - ['2:03', Daversa trades phrases with band]
    - ['2:29', 'Controlled chaos: everyone plays at once']
    - ['2:42', 'Daversa plays the first phrase of tune’s melody, transposing up']
    - ['3:30', 'Chaos subsides, violin & piano play tune "western" style']
    - ['3:48', 'Drum fill, band in. Sax soli + brass hits']
    - ['4:17', Band hits alternate with short trumpet solos]
    - ['4:38', '"Don''t fence me in!"']
  notes: |-
    - Cole Porter wrote this tune in 1934 as a cowboy tune. In the 2018 context, it references the detention of immigrants at the southern border of the US, giving the lyrics a more political, urgent meaning.
---
