---
title: String Players (& a Drummer)
description: A survey of contemporary jazz guitarists, string players, and a prominent drummer.
order: 33
media:
  mary-halvorson:
    youtube: kXvs5_rLU9g
    title: 'Mary Halvorson video'
  regina-carter:
    youtube: CFDq3bwgFJU
    title: 'Regina Carter video'
intro: |-
  In this final unit for the module, we look at some notable guitarists and other string players who have helped define contemporary jazz, as well as one of the most influential drummers working today. From guitarists who seamlessly integrate modern harmony and global influences, to a violinist combining classical technique with jazz improvisation, to a drummer who blends folk elements with modern jazz aesthetics, these artists highlight the continued evolution and diversification of jazz.
sections:
- heading: Kurt Rosenwinkel
  artists:
  - name: Kurt Rosenwinkel
    born: 1970
  prose: |-
    Kurt Rosenwinkel is one of the most influential guitarists of his generation. He often uses a distinct, modern tone and sometimes sings along with his solos via a lapel mic. He attended Berklee College of Music (though he left before graduating to tour with Gary Burton). He has worked with Larry Goldings, Joe Henderson, Mark Turner, and was the first guitarist in Brian Blade’s Fellowship Band.

    In addition to original compositions, Rosenwinkel also formed a Standards Trio around 2009, focusing on classic tunes. His approach to standards is both respectful of tradition and uniquely modern.
  tracks:
  - title: Reflections
    performer: Kurt Rosenwinkel Trio
    composer: Thelonious Monk
    recorded: '2009'
    album: Reflections
    personnel:
    - [Kurt Rosenwinkel, guitar]
    - [Eric Revis, bass]
    - [Eric Harland, drums]
    spotify: 7c7aRF6EJ96zCmep9BmewW
    intro: |-
      Form: 32-bar AABA
    guidenote: |-
      **Form:** 32-bar AABA
    guide: |-
      0:00 | Solo guitar intro
      0:52 | A, head in
      1:26 | A
      2:01 | B (band plays composed rhythmic figure)
      2:37 | A
      3:11 | AA, bass (Revis) solo
      4:24 | BA, Rosenwinkel solo
      5:37 | AA, Rosenwinkel solo continues, switch to double-time feel
      6:48 | B, head out. Still double-time.
      7:25 | A, back to original 2-feel
      7:50 | Extended guitar cadenza
      8:53 | Final chord
  notes: |-
    - Watch the trio in the studio on YouTube.
    - The tune was originally recorded by Monk in 1952.
- heading: Lionel Loueke
  artists:
  - name: Lionel Loueke
    born: 1973
  prose: |-
    Lionel Loueke is from Benin, West Africa. He began guitar at age 17 and eventually studied at Berklee and the Monk Institute. He was voted top Rising Star guitarist in DownBeat multiple times.

    Loueke has played with Herbie Hancock, contributing to the Grammy-winning *River: The Joni Letters* . He often incorporates African rhythmic and melodic elements, including tongue clicks inspired by South African languages.
  tracks:
  - title: Freedom Dance
    performer: Lionel Loueke Trio
    composer: Lionel Loueke
    recorded: '2011'
    album: Heritage
    personnel:
    - [Lionel Loueke, 'guitar, vocals']
    - [Derrick Hodge, bass]
    - [Mark Guiliana, drums]
    spotify: 3wSD488adiUFtKzxHb8fmi
    guide: |-
      0:00 | Intro
      0:17 | Head in
      1:19 | Loueke (g) solo
      3:59 | Head out
  notes: |-
    - Watch a live version on YouTube with a slightly different band.
    - Robert Glasper produced *Heritage* .
    - Derrick Hodge’s complex, syncopated bass lines exemplify a modern approach to bass playing.
- heading: Mary Halvorson
  artists:
  - name: Mary Halvorson
    born: 1980
  prose: |-
    Mary Halvorson is a guitarist and composer. She won DownBeat’s best guitarist several years running and a 2019 MacArthur fellowship. She teaches at The New School. Her style is often avant-garde, blending experimental rock, folk, and indie influences with jazz. She’s known for pitch-bending electronics and balancing delicate lines with "sprays of atmospheric noise."
  tracks:
  - title: Cascades
    performer: Mary Halvorson
    composer: Oliver Nelson
    recorded: November 4 & 5, 2014
    album: (From her solo repertoire)
    credits:
    - '**Note:**

      Original by Oliver Nelson is a 12-bar minor blues.'
    personnel:
    - [Mary Halvorson, guitar (solo)]
    spotify: 1nN3eCjkpz4U84vLrzp5vt
    intro: |-
      Form: the original is a 12-bar minor blues.

      Here's a video of a live performance by Halvorson. She plays "Cascades" at 16:56.

      @video(mary-halvorson)
    guide: |-
      0:00 | Intro
      0:40 | Head in (melody of “Cascades”)
      1:29 | Halvorson solos, mixing melody fragments, distorted chords
      3:42 | Head out, with silence between phrases
  notes: |-
    - Original “Cascades” is on Oliver Nelson’s *Blues and the Abstract Truth* .
    - Halvorson’s tone often resembles rock or metal more than jazz guitar.
- heading: Julian Lage
  artists:
  - name: Julian Lage
    born: 1987
  prose: |-
    Julian Lage was a child prodigy. The documentary *Jules at Eight* featured him as a child. He studied at various institutions including Berklee. His first album, *Sounding Point* , was nominated for a Grammy in 2010 (Best Contemporary Jazz Album).

    Lage’s music often blends jazz with Americana and bluegrass influences. On “The Informant,” he collaborates with Bela Fleck (banjo) and Chris Thile (mandolin), both notable for their jazz/bluegrass fusion.

    ### Béla Fleck (1958-)

    Béla Fleck is a banjo player known for his group The Flecktones, a jazz/bluegrass fusion band.

    ### Chris Thile (1981-)

    Chris Thile is a mandolinist, also a former child prodigy, member of Nickel Creek, and host of NPR’s *Live From Here* .
  tracks:
  - title: The Informant
    performer: Julian Lage with Béla Fleck & Chris Thile
    composer: Julian Lage, Béla Fleck, Chris Thile
    recorded: May/June 2008
    album: Sounding Point
    personnel:
    - [Julian Lage, guitar]
    - [Béla Fleck, banjo]
    - [Chris Thile, mandolin]
    spotify: 2baaMl7yxpsLnFHlpk2RiN
    guide: |-
      0:00 | Rubato intro
      1:09 | Fleck sets tempo
      1:25 | Thile adds percussive strum
      1:39 | Harmonic shift, Lage plays fast lines
      1:54 | All three trade melodic phrases, interlocking rhythms
      2:46 | Return to composed material, deconstruct to end
  notes: |-
    - The piece feels as much bluegrass as jazz. The trio locks in tight, groove-like interplay without drums.
    - *Sounding Point* was nominated for a Grammy for Best Contemporary Jazz Album (2010).
- heading: Regina Carter
  artists:
  - name: Regina Carter
    born: 1966
  prose: |-
    Regina Carter is a violinist who trained classically (Suzuki method) and later pivoted to jazz at New England Conservatory. She won a MacArthur fellowship in 2006 and has explored classical, jazz, and various other styles. Her 2016 album *Accentuate the Positive* is dedicated to Ella Fitzgerald.

    @video(regina-carter)
  tracks:
  - title: Crying in the Chapel
    performer: Regina Carter Ensemble
    composer: Artie Glenn
    recorded: December 2016
    album: Accentuate the Positive
    personnel:
    - [Xavier Davis, keyboards]
    - [Marvin Sewell, guitar]
    - [Regina Carter, violin]
    - [Chris Lightcap, bass]
    - [Alvester Garnett, drums]
    spotify: 28C1ARdE17cXwZmvGwgUCA
    guide: |-
      0:00 | Intro groove
      0:17 | Head in
      0:45 | Groove interlude
      1:04 | Head again
      1:34 | Bridge
      2:08 | Carter solo over Em–Cm vamp
      2:59 | New chords
      3:12 | Davis (e-p) solo
      4:18 | Head out from bridge
  notes: |-
    - Famous from Elvis Presley’s version, here it’s transformed into a soulful jazz piece.
    - Carter learned largely by ear and recently took music theory lessons to fill that gap.
    - She has shown interest in music therapy as a future path.
- heading: Brian Blade
  artists:
  - name: Brian Blade
    born: 1970
  prose: |-
    Brian Blade is a composer and one of the most sought-after drummers in jazz. He studied at Loyola University in New Orleans and has worked with a who’s who of jazz, including Joshua Redman, Christian McBride, Chick Corea, Pat Metheny, and Wayne Shorter.

    In 1998, with pianist Jon Cowherd, he formed the Fellowship Band (which once included Kurt Rosenwinkel). The group’s sound blends jazz with pygmy chants, American folk, and spiritual elements. Their album *Landmarks* (2014) exemplifies this holistic style.
  tracks:
  - title: Landmarks
    performer: Brian Blade & The Fellowship Band
    recorded: Feb 1-4, 2012
    album: Landmarks
    personnel:
    - [Melvin Butler, soprano & tenor saxophone]
    - [Myron Walden, 'alto saxophone, bass clarinet']
    - [Jon Cowherd, piano]
    - [Marvin Sewell, guitar]
    - [Chris Thomas, bass]
    - [Brian Blade, drums]
    spotify: 5bjV2B1rH2gkdtJ4qr0qQy
    guide: |-
      0:00 | Intro, bass plays melody freely
      2:19 | Sax takes theme
      2:50 | Butler (sop) solo
      3:38 | Composed melody over new changes
      4:07 | Walden (b-cl) states main theme
      4:33 | Cowherd (p) solo, sparse at first, band joins gradually
      7:31 | Walden & Cowherd play composed melody, soft finish
  notes: |-
    - Watch them perform “Landmarks” on KEXP (YouTube) for a live perspective.
---
